...
import { getRuleOrPercent,intervalMainPage,balanceInterval } from '../../api/home/homePage'
        ...
        checkLotteryRule(id,name,item){ //  点击二级彩种
            ...
            getRuleOrPercent(data).then( (res) => {  //  获取，存储当前点击的一级玩法
                if(res.data.success == 1){
                    ...
                    this.$router.push(`/lotteryEnter?id=${id}&par=${item.id}&name=${item.name}`)
                }else{
                    this.$tips.info(par.respMsg);
                }
